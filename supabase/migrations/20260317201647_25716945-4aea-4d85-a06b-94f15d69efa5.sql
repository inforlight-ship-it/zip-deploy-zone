...
-- =============================================
-- MULTI-TENANT HUB TABLES (STEP 2)
-- =============================================
...
CREATE POLICY "Superadmins can manage plans" ON public.subscription_plans
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    ...
    JOIN public.user_tenant_roles utr ON utr.user_tenant_id = ut.id
    JOIN public.roles r ON r.id = utr.role_id
    WHERE ut.user_id = _user_id AND ut.tenant_id = _tenant_id AND ut.is_active = true AND r.name = _role
  )
$$;
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.user_tenants WHERE user_id = _user_id AND tenant_id = _tenant_id AND is_active = true
  )
$$;

-- 19. UPDATE handle_new_user
...
INSERT INTO public.permissions (resource, action, description) VALUES
  ...
  ('diagnostico', 'view', 'Visualizar diagnóstico'),
  ('diagnostico', 'manage', 'Gerenciar diagnóstico'),
  ('mapeamento', 'view', 'Visualizar mapeamento'),
  ('mapeamento', 'manage', 'Gerenciar mapeamento'),
  ('documentos', 'view', 'Visualizar documentos'),
  ('documentos', 'manage', 'Gerenciar documentos'),
  ('solicitacoes', 'view', 'Visualizar DSR'),
  ('solicitacoes', 'manage', 'Gerenciar DSR'),
  ('consentimento', 'view', 'Visualizar consentimentos'),
  ('consentimento', 'manage', 'Gerenciar consentimentos'),
  ('incidentes', 'view', 'Visualizar incidentes'),
  ('incidentes', 'manage', 'Gerenciar incidentes'),
  ('auditoria', 'view', 'Visualizar auditorias'),
  ('auditoria', 'manage', 'Gerenciar auditorias'),
  ('fornecedores', 'view', 'Visualizar fornecedores'),
  ('fornecedores', 'manage', 'Gerenciar fornecedores'),
  ('relatorios', 'view', 'Visualizar relatórios'),
  ('relatorios', 'manage', 'Gerenciar relatórios'),
  ('dpo', 'view', 'Visualizar portal DPO'),
  ('dpo', 'manage', 'Gerenciar portal DPO');
